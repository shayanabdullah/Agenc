  ...
  return (
    <>
      ...
      <motion.nav
        ...
        whileInView="show"
        viewport={{ once: true }}
        className={`fixed top-0 left-0 z-20 w-full md-2:block hidden transition-all duration-300 ease-in-out
        ${isSticky ? 'bg-[#0d0d4c] py-5' : 'bg-transparent py-[30px]'}
        `} >
      ...
      {/* mobile navber */}
      <section>
        <nav className={`fixed top-0 left-0  py-5 w-full z-98 md-2:hidden  ${isSticky ? 'bg-[#0d0d4c]' : 'bg-transparent'} transition-colors duration-300 ease-in-out`}>
          <div className="container">
            <div className="menu-main flex justify-between items-center w-full px-2">
              ...
              <div className="menubar cursor-pointer text-white text-5xl" onClick={() => setMenuOpen(true)}>
                <RiMenu3Fill />
        ...
        <div className="sidebar-main relative">
          <div className={`sidebar h-screen bg-[#3d346b] z-99 fixed top-0 right-0 transition-all ease-in-out duration-500 opacity-0 w-0  max-w-[300px]  ${MenuOpen === true ? 'w-full opacity-100' : ''} `}>
